...
Coroutine testAllRedisCommands(IOScheduler* scheduler)
{
    ...
    auto ping_result = co_await client.ping();
    ...
    auto echo_result = co_await client.echo("Hello Redis!");
    ...
    auto set_result = co_await client.set("test_string_key", "test_value");
    ...
    auto get_result = co_await client.get("test_string_key");
    ...
    auto exists_result = co_await client.exists("test_string_key");
    ...
    auto setex_result = co_await client.setex("test_expire_key", 10, "expire_value");
    ...
    auto incr_result = co_await client.incr("test_counter");
    ...
    auto decr_result = co_await client.decr("test_counter");
    ...
    auto del_result = co_await client.del("test_string_key");
    ...
    auto hset_result = co_await client.hset("test_hash", "field1", "value1");
    ...
    // 再添加一个字段
    co_await client.hset("test_hash", "field2", "value2");

    // 测试HGET
    std::cout << "12. Testing HGET..." << std::endl;
    auto hget_result = co_await client.hget("test_hash", "field1");
    ...
    auto hgetall_result = co_await client.hgetAll("test_hash");
    ...
    auto hdel_result = co_await client.hdel("test_hash", "field1");
    ...
    auto lpush_result = co_await client.lpush("test_list", "item1");
    ...
    auto rpush_result = co_await client.rpush("test_list", "item2");
    ...
    // 再添加一些元素
    co_await client.rpush("test_list", "item3");
    co_await client.rpush("test_list", "item4");

    // 测试LLEN
    std::cout << "17. Testing LLEN..." << std::endl;
    auto llen_result = co_await client.llen("test_list");
    ...
    auto lrange_result = co_await client.lrange("test_list", 0, -1);
    ...
    auto lpop_result = co_await client.lpop("test_list");
    ...
    auto rpop_result = co_await client.rpop("test_list");
    ...
    auto sadd_result = co_await client.sadd("test_set", "member1");
    ...
    // 添加更多成员
    co_await client.sadd("test_set", "member2");
    co_await client.sadd("test_set", "member3");

    // 测试SCARD
    std::cout << "22. Testing SCARD..." << std::endl;
    auto scard_result = co_await client.scard("test_set");
    ...
    auto smembers_result = co_await client.smembers("test_set");
    ...
    auto srem_result = co_await client.srem("test_set", "member1");
    ...
    auto zadd_result = co_await client.zadd("test_zset", 1.0, "member1");
    ...
    // 添加更多成员
    co_await client.zadd("test_zset", 2.0, "member2");
    co_await client.zadd("test_zset", 3.0, "member3");
    co_await client.zadd("test_zset", 4.0, "member4");

    // 测试ZSCORE
    std::cout << "26. Testing ZSCORE..." << std::endl;
    auto zscore_result = co_await client.zscore("test_zset", "member2");
    ...
    auto zrange_result = co_await client.zrange("test_zset", 0, -1);
    ...
    auto zrem_result = co_await client.zrem("test_zset", "member1");
    ...
    auto pipeline_result = co_await client.pipeline(pipeline_commands);
    ...
    co_await client.del("test_counter");
    co_await client.del("test_expire_key");
    co_await client.del("test_hash");
    co_await client.del("test_list");
    co_await client.del("test_set");
    co_await client.del("test_zset");
...
Coroutine testExecuteCommand(IOScheduler* scheduler)
{
    ...
    // 测试 MSET
    auto mset_result = co_await client.execute("MSET", {"key1", "value1", "key2", "value2"});
    if (mset_result && mset_result.value()) {
        std::cout << "   [PASSED] EXECUTE MSET succeeded" << std::endl;
    }

    // 测试 MGET
    auto mget_result = co_await client.execute("MGET", {"key1", "key2"});
    ...
    // 清理
    co_await client.execute("DEL", {"key1", "key2"});
