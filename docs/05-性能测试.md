...
if (auto __await_result = co_await client.pipeline(commands); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
```cpp
// 快速操作
if (auto __await_result = co_await client.get("key").timeout(std::chrono::seconds(1)); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}

// 批量操作
if (auto __await_result = co_await client.pipeline(batch).timeout(std::chrono::seconds(30)); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}

// 非关键操作可以不设超时
if (auto __await_result = co_await client.set("cache_key", "value"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
