...
```cpp
// 仅密码
if (auto __await_result = co_await client.connect("127.0.0.1", 6379, "", "password"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
```cpp
if (auto __await_result = co_await client.connect("127.0.0.1", 6379, "", "", 1); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
} // 选择 DB 1
```
...
if (auto __await_result = co_await client.connect("redis://username:password@127.0.0.1:6379/1"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
