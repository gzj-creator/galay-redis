...
```cpp
// 参数方式
if (auto __await_result = co_await client.connect("127.0.0.1", 6379, "username", "password"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}

// URL 方式
if (auto __await_result = co_await client.connect("redis://:password@127.0.0.1:6379/0"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}

// 连接也支持超时
if (auto __await_result = co_await client.connect("127.0.0.1", 6379).timeout(std::chrono::seconds(3)); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
```cpp
if (auto __await_result = co_await sub.subscribe("channel"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
if (auto __await_result = co_await pub.publish("channel", "message"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
# 性能测试
./build/test/T7-redis_client_benchmark 10 100

# Pipeline 性能测试
./build/test/T7-redis_client_benchmark 20 5000 pipeline 100
```
