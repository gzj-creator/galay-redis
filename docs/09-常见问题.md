...
```cpp
// 确保密码正确
if (auto __await_result = co_await client.connect("127.0.0.1", 6379, "", "correct_password"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
2. 定期 ping 保活：
   ```cpp
   if (auto ping = co_await client.ping(); !ping) {
       std::cerr << "ping 失败\n";
   }
   ```
3. 调整 Redis 服务器配置：
...
2. 是否选择了正确的数据库：
   ```cpp
   if (auto select_res = co_await client.select(0); !select_res) {  // 选择 DB 0
       std::cerr << "select 失败\n";
   }
   ```
...
if (auto __await_result = co_await client.execute("EXEC", {}); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
```cpp
if (auto __await_result = co_await client.subscribe("channel"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
```cpp
if (auto __await_result = co_await client.psubscribe("news.*"); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
...
// 从 Sentinel 自动发现主从拓扑
if (auto __await_result = co_await ms.refreshFromSentinel(); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
...
// 按 key 自动路由
if (auto __await_result = co_await cluster.executeByKey("key", "SET", {"key", "value"}); !__await_result) {
    // 错误处理：记录日志、重试或提前返回
}
```
